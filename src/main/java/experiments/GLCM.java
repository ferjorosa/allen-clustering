...
public class GLCM {
    ...
    public static Tuple4<IModelWithScore, Double, Double, Long> run(DataOnMemory<DataInstance> trainData,
        ...
        System.out.println("Total time: " + learnTime + " ms");
    ...
    private static IModelWithScore learnToMaxCardinality(Geast geast, List<Variable> nonClassVariables, LogUtils.LogLevel logLevel) {
        ...
        for(int card = 2; card < Integer.MAX_VALUE; card++) {
            ...
            Gltm lcm = Builder.buildNaiveBayesModel(
                    new Gltm(),
                    new DiscreteVariable(card),
                    nonClassVariables);

            geast.context().parameterGenerator().generate(lcm);
            IModelWithScore result = geast.context().estimationEm().estimate(lcm);
            long endTime = System.currentTimeMillis();
            long learnTime = (endTime - initTime);
