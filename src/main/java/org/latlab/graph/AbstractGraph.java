/**
 * AbstractGraph.java Copyright (C) 2006 Tao Chen, Kin Man Poon, Yi Wang, and
 * Nevin L. Zhang
 */
...
public abstract class AbstractGraph implements Cloneable {
     ...
     */
    private Map<String, AbstractNode> _names;
     ...
     */
    public abstract int depthFirstSearch(
        AbstractNode node, int time, Map<AbstractNode, Integer> d,
        Map<AbstractNode, Integer> f);
    ...
    /**
     * Returns a set containing names of the nodes.
     * 
     * @return names of the nodes
     */
    ...
    /**
     * Adds to the name map with key of {@code name} and value of {@code node}.
     * 
     * @param name
     *            name of the node
     * @param node
     *            the node to associated with the name
     */
    public void putNode(String name, AbstractNode node) {
        _names.put(name, node);
    }

    /**
     * Removes the key with the given {@code name} from the name map.
     * 
     * @param name
     *            key to remove
     * @return the node if one is associated with the key or {@code null}
     *         otherwise
     */
    public AbstractNode removeNodeKey(String name) {
