...
public class VBEM_Local {
    ...
    public double learnModel(PlateuStructure model, DAG dag, List<Variable> variables) {
    ...
    private List<Node> initialization(PlateuStructure model, DAG dag, List<Variable> variables) {
    ...
    private List<Node> computeLocalNodes(List<Variable> variables, DAG dag) {
    ...
    private void pyramidInitialization(List<Node> localNodes, int initialNumberOfCandidates, int maxIterations, boolean withConvergence) {
        ...
        List<Tuple<CompoundVector, Double>> candidates = new ArrayList<>(initialNumberOfCandidates);
        ...
        for(int i = 0; i < initialNumberOfCandidates; i++) {
            ...
            candidates.add(new Tuple<>(this.plateuStructure.getLatentVariablesPosterior(localNodes), this.plateuStructure.getLogProbabilityOfEvidence()));
        }
        /* Pyramidal iteration */
        while(candidates.size() > 1) {
            ...
            List<Tuple<CompoundVector, Double>> auxListOfCandidates = new ArrayList<>(candidates.size());
