...
public class IslandFinder {
    ...
    public IslandFinder(int maxCardinality) {
    ...
    public DAG generate(DataOnMemory<DataInstance> data, double[][] mutualInformations, Map<String, double[]> priors) {
        ...
        List<Tuple<List<Integer>, Integer>> islands = new ArrayList<>();
        ...
        while(attributeIndexSet.size() >= 2) {
            ...
            Tuple<Integer, Integer> highestMIattributesIndexes = highestMIindexes(attributeIndexSet, mutualInformations);
            ...
            do {
                ...
                if(closestAttributeToTheGroup != -1) {
                    ...
                    /* UD Test */
                    Tuple<Boolean, Integer> udTest = unidimensionalityTest(groupIndexes, closestAttributeToTheGroup, data, priors);
                    if (udTest.getFirst()) {
            ...
            islands.add(new Tuple<>(groupIndexes, islandLatentVarCardinality));
        }
     ...
     */
    private Tuple<Integer, Integer> highestMIindexes(Set<Integer> attributeIndexSet, double[][] mutualInformations) {
        int best_i = -1;
        int best_j = -1;
        double best_value = -1;
        ...
        return new Tuple<>(best_i, best_j);
    }
    ...
    private Tuple<Boolean, Integer> unidimensionalityTest(List<Integer> groupIndexes, int closestAttributeToTheGroup, DataOnMemory<DataInstance> data, Map<String, double[]> priors) {
        ...
        if(result2LTM.getElbo() > resultLCM.getElbo()
            ...
            return new Tuple<>(false, resultLCM.getDag().getVariables().getVariableByName("LV").getNumberOfStates());
        }

        /* Si pasa el UD-Test devolvemos true y la cardinalidad de la variable latente del LCM */
        return new Tuple<>(true, resultLCM.getDag().getVariables().getVariableByName("LV").getNumberOfStates());
    }
    ...
    private Result learn2LTM(DataOnMemory<DataInstance> data, Map<String, double[]> priors) {
        ...
        expansionOperators.add(new BltmHcIncreaseCard(this.maxCardinality));
        expansionOperators.add(new BltmHcAddNode(2));
        adjustmentOperators.add(new BltmHcRelocateNode());
    ...
    private int learnCardinality(Set<Integer> attributeIndexSet, DataOnMemory<DataInstance> data, Map<String, double[]> priors) {
        ...
        Set<BltmHcOperator> operators = new LinkedHashSet<>();
        operators.add(new BltmHcIncreaseCard(10));
    ...
    /** Special case for when an attribute hasn't been located in any island. We move it to the closest island */
    private void specialCase(List<Tuple<List<Integer>, Integer>> islands, Integer attributeIndex, double[][] mutualInformations) {
        ...
        for(Tuple<List<Integer>, Integer> island: islands)
            if(island.getFirst().contains(closestAttribute))
                island.getFirst().add(attributeIndex);
    }

    private DAG generateIslandsDAG(List<Tuple<List<Integer>, Integer>> islands, DataOnMemory<DataInstance> data) {
