package voltric.clustering.multidimensional.old.attribute;
...
public class KeivaniFinder implements AttributeGrouping {
    ...
    public List<HLCM> find(DiscreteData dataSet) {
        ...
        while(variableSet.stream().filter(x->x.isManifestVariable()).count() > 0){
            ...
            Tuple<DiscreteVariable, DiscreteVariable> bestPair = findBestPair(variableSet, pairScores);
            ...
            if(bestPair.getFirst().isManifestVariable() && bestPair.getSecond().isManifestVariable()){
                ...
                HLCM newCluster = HlcmCreator.createLCM(clusterSet, 2, new Random());
    ...
    private Tuple<DiscreteVariable, DiscreteVariable> findBestPair(List<DiscreteVariable> variableSet,
    ...
    private void updatePairScoresWithLV(Map<DiscreteVariable, Map<DiscreteVariable, Double>> pairScores,
        ...
        // TODO: Hasta que no arregle el tema de las LVs y el calculo de la MI y entropia de forma frequentista, esto no funciona
